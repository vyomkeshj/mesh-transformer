...
network.state = read_ckpt(network.state, "/home/jha0007/bigdiks/slim_model/gpt_sql_pile_wsql/", 8, shards_out=cores_per_replica)
...
def infer(context, top_k=40, top_p=0.9, temp=1.0, gen_len=512):
    ...
    start = time.time()
    output = network.generate(batched_tokens,
                              length,
                              gen_len,
                              {"top_p": np.ones(per_replica_batch) * top_p,
                               "top_k": top_k is not None and (np.ones(per_replica_batch, dtype=np.int32) * top_k) or None,
                               "temp": np.ones(per_replica_batch) * temp})
